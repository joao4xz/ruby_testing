...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_energy_drink) { described_class.new('energy_drink') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_energy_drink.type).to eq('energy_drink')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_energy_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:orange_juice) { described_class.new('orange_juice') }
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:protein_shake) { described_class.new('protein_shake', 15) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(protein_shake).not_to be_full
      end
    end
  end
end
